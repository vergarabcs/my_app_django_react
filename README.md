...
- [ ] Create a new virtualenv with either [virtualenvwrapper](https://virtualenvwrapper.readthedocs.io/en/latest/) or only virtualenv: `mkvirtualenv wordFactory` or `python -m venv wordFactory-venv`
...
- [ ] Make sure the virtualenv is activated `workon wordFactory` or `source wordFactory-venv/bin/activate`
- [ ] Compile the requirements before installation and install them:  `make compile_install_requirements`
- [ ] Change the first line of README to the name of the project
- [ ] Add an email address to the `ADMINS` settings variable in `wordFactory/backend/wordFactory/settings/base.py`
- [ ] Change the `SERVER_EMAIL` to the email address used to send e-mails in `wordFactory/backend/wordFactory/settings/production.py`
...
- Create a copy of ``wordFactory/settings/local.py.example``:  
  `cp wordFactory/settings/local.py.example wordFactory/settings/local.py`
...
- Deregister the example app by removing `'exampleapp.apps.ExampleappConfig'` from ``backend/wordFactory/settings/base.py``.
- Adjust ``backend/wordFactory/urls.py`` to point to your newly created Django app and remove the path configuration that redirects to the deleted example app.
